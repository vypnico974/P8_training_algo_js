...
if the album does not have a tracks property, assign it an empty array. Then add the value as the
last item in the album's tracks array.
Note: A copy of the recordCollection object is used for the tests. You should not directly
modify the recordCollection object.*/
// Setup
...
function updateRecords(records, id, prop, value) {
   ...
   if (array[id]){
   if (value ==""){
       delete array[id][prop];
       return array;
   }
   if ( (prop == "artist") && (value != "")){
     array[id].artist = value;
     return array;
   }
   if ( (prop == "tracks") && (value != "Free")){
     array[id].tracks = [value];
     return array;
   }
   if ( (prop == "albumTitle") && (value != "")){
     array[id].albumTitle = value;
     return array;
   }
